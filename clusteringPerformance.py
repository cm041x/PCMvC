...
def clusteringMetrics(trueLabel, predictiveLabel):
    ...
    return NMI, Purity, ARI, Fscore, Precision, Recall
...
def spectral_clustering(points, k, gnd, repnum=10):
    ...
    W = (np.abs(points) + np.abs(points.T))/2
    ...
    [ACC, NMI, Purity, ARI, Fscore, Precision, Recall] = StatisticClustering2(k_smallest_eigenvectors, gnd, k)
...
def StatisticClustering2(features, gnd, clusterNum):
    ### Input the mean and standard diviation with 10 experiments
    repNum = 10
    ACCList = np.zeros((repNum, 1))
    NMIList = np.zeros((repNum, 1))
